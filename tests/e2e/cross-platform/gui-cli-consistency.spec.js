...
test.describe("GUI與CLI功能一致性測試", () => {
  ...
  test("功能覆蓋度一致性", async () => {
    ...
    // 新增：檢查 alias 與清單命令
    const aliasResult = await executeCLI(["run", "--help"]);
    expect(aliasResult.exitCode).toBe(0);

    const promptList = await executeCLI(["prompt", "list"]);
    expect(promptList.exitCode).toBe(0);

    const jobList = await executeCLI(["job", "list"]);
    expect(jobList.exitCode).toBe(0);
