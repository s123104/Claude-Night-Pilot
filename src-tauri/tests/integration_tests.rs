...
use claude_night_pilot_lib::interfaces::{cli_adapter::CLIAdapter, tauri_adapter::TauriAdapter};
use claude_night_pilot_lib::services::{
    health_service::HealthService, job_service::JobService, prompt_service::PromptService,
...
async fn setup_test_environment() -> TestEnvironment {
    ...
    let prompt_service = Arc::new(
        PromptService::new()
            .await
            .expect("Failed to create prompt service"),
    );
    let job_service = Arc::new(
        JobService::new()
            .await
            .expect("Failed to create job service"),
    );
    let health_service = Arc::new(HealthService::new());
    let sync_service = Arc::new(SyncService::new());

    let tauri_adapter = Arc::new(
        TauriAdapter::new()
            .await
            .expect("Failed to create Tauri adapter"),
    );
...
async fn test_complete_prompt_lifecycle() {
    ...
    let tauri_result = env
        .tauri_adapter
        .gui_create_prompt(create_request.clone())
        .await;
...
async fn test_gui_cli_synchronization() {
    ...
    for prompt_request in gui_prompts {
        let api_response = env
            .tauri_adapter
            .gui_create_prompt(prompt_request)
            .await
            .unwrap();
        assert!(api_response.success);
...
async fn test_concurrent_operations() {
    ...
    // 4. 併發讀取測試
    let read_handles: Vec<_> = results
        .iter()
        .map(|&prompt_id| {
            let prompt_service = Arc::clone(&env.prompt_service);
