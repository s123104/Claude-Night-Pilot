...
async fn setup_performance_env() -> PerformanceTestEnv {
    ...
    let cli_adapter = Arc::new(
        CLIAdapter::new()
            .await
            .expect("Failed to create CLI adapter"),
    );
...
async fn test_cli_response_time() {
    ...
    // 目標: CLI 命令應在100ms內響應
    assert!(
        health_time < Duration::from_millis(100),
        "CLI 響應時間過長: {:?}, 目標: < 100ms",
        health_time
    );
...
async fn test_database_query_performance() {
    ...
    // 目標: 單次查詢應在50ms內完成
    assert!(
        single_query_time < Duration::from_millis(50),
        "單次查詢時間過長: {:?}, 目標: < 50ms",
        single_query_time
    );
    ...
    // 目標: 列表查詢應在200ms內完成
    assert!(
        list_query_time < Duration::from_millis(200),
        "列表查詢時間過長: {:?}, 目標: < 200ms",
        list_query_time
    );
...
async fn test_concurrent_performance() {
    ...
    println!("- 最長任務時間: {:?}", task_times.iter().max().unwrap());
    ...
    let final_prompts: Vec<serde_json::Value> =
        serde_json::from_str(&final_prompts_str).unwrap_or_default();
...
async fn test_memory_usage() {
    ...
    println!(
        "清理後記憶體使用: {} MB",
        after_cleanup_memory / 1024 / 1024
    );
}
    ...
    while stability_start.elapsed() < test_duration {
        ...
        let result = match operation_type {
            0 => env
                ...
                .await
                .map(|_| ()),
            1 => env.cli_adapter.cli_list_prompts("json").await.map(|_| ()),
            _ => env
                .cli_adapter
                .cli_health_check("json", true)
                .await
...
async fn generate_performance_report() {
    ...
    println!(
        "   - 最快啟動時間: {:?}",
        startup_times.iter().min().unwrap()
    );
    println!(
        "   - 最慢啟動時間: {:?}",
        startup_times.iter().max().unwrap()
    );
    ...
    let operations = vec![
        ("創建 Prompt", {
            ...
            let _ = env
                .cli_adapter
                .cli_create_prompt("基準測試".to_string(), "基準測試內容".to_string(), None)
                .await
                .unwrap();
            start.elapsed()
        }),
    ...
    println!(
        "   - 吞吐量: {:.2} ops/sec",
        100.0 / total_concurrent.as_secs_f64()
    );
