...
fn benchmark_database_initialization(c: &mut Criterion) {
    ...
    group.bench_function("database_manager_new", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            ...
            let _db = DatabaseManager::new(db_path.to_str().unwrap()).await.unwrap();
        });
    });
...
fn benchmark_crud_operations(c: &mut Criterion) {
    ...
    group.bench_function("create_prompt_sync", |b| {
        ...
        b.iter(|| {
            ...
            let content = format!("基準測試內容 {} - 這是一個相對長的測試內容，用於模擬實際使用情況", counter);
...
fn benchmark_schedule_operations(c: &mut Criterion) {
    ...
    group.bench_function("get_pending_schedules", |b| {
        ...
        b.to_async(&rt).iter(|| {
            let db = db.clone();
            async move {
                black_box(db.get_pending_schedules_async().await.unwrap())
            }
        });
    });
...
fn benchmark_token_operations(c: &mut Criterion) {
    ...
    group.bench_function("update_token_stats", |b| {
        ...
        b.to_async(&rt).iter(|| {
            ...
            async move {
                black_box(
                    db.update_token_usage_stats_async(
                        1000,
                        500,
                        0.05
                    ).await.unwrap()
    ...
    group.bench_function("get_token_stats", |b| {
        ...
        b.to_async(&rt).iter(|| {
            let db = db.clone();
            async move {
                black_box(db.get_token_usage_stats_async().await.unwrap())
            }
        });
    });
...
fn benchmark_system_operations(c: &mut Criterion) {
    ...
    group.bench_function("health_check", |b| {
        ...
        b.to_async(&rt).iter(|| {
            let db = db.clone();
            async move {
                black_box(db.health_check().await.unwrap())
            }
        });
    });
