...
impl JobExecutor {
    ...
    pub async fn execute_job(&self, job: &Job) -> Result<JobExecutionResult> {
        ...
        // 執行任務 (包含重試邏輯)
        let result = self
            .execute_with_retry(job, &mut execution, unified_options)
            .await;
        ...
        // 更新統計信息
        self.update_execution_stats(result.is_ok(), total_duration_ms as f64 / 1000.0)
            .await;
    ...
    async fn execute_with_retry(
        ...
        loop {
            ...
            execution.progress =
                Some((retry_count as f32 / (max_retries + 1) as f32 * 100.0) as u8);
            ...
            match self.execute_claude_task(job, &unified_options).await {
                ...
                Err(e) => {
                    ...
                    execution.current_step =
                        Some(format!("等待重試 ({} 秒後重試)", delay.as_secs()));
    ...
    async fn execute_claude_task(
        ...
        // 檢查資源限制
        self.check_resource_limits(&job.execution_options.resource_limits)
            .await?;
    ...
    async fn update_execution_stats(&self, success: bool, execution_time: f64) {
        ...
        // 更新平均執行時間
        let total_time =
            stats.average_execution_time * (stats.total_executions - 1) as f64 + execution_time;
