...
impl JobScheduler {
                            ...
                            Ok(Some(job_details)) => {
                                match callback.execute_job(&job_details).await {
                                    ...
                                    Err(e) => {
                                        ...
                                        callback
                                            .on_job_complete(&job_id, false, Some(error_msg))
                                            .await
                                    }
                                }
                            }
    ...
    pub async fn get_scheduler_state(&self) -> SchedulerState {
        ...
        state.active_jobs = tracker
            .values()
            .filter(|s| s.status == JobStatus::Active)
            .count();
        state.paused_jobs = tracker
            .values()
            .filter(|s| s.status == JobStatus::Paused)
            .count();
