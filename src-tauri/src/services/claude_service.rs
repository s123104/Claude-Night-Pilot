...
use crate::models::{
    ClaudeRequest, ClaudeResponse, ExecutionResult, ExecutionStatus, PerformanceMetrics,
    SecurityAudit,
};
...
impl ClaudeService {
    ...
    /// 提交執行請求
    pub async fn submit_execution(&self, request: ClaudeRequest) -> Result<String> {
        // 檢查並行執行限制
        ...
        self.active_executions
            .write()
            .await
            .insert(request.id.clone(), execution);
    ...
    pub async fn cancel_execution(&self, request_id: &str) -> Result<()> {
        ...
        if let Some(mut execution) = executions.remove(request_id) {
            ...
            let result = ExecutionResult::new(ClaudeRequest::new("已取消的請求"), response);
...
mod tests {
    ...
    async fn test_execution_submission() {
        let service = ClaudeService::with_defaults();
        let request = ClaudeRequest::new("測試提示").with_source(RequestSource::Cli);
    ...
    async fn test_security_audit() {
        ...
        let audit = service
            .perform_security_audit(&sensitive_request)
            .await
            .unwrap();
