...
use crate::services::job_executor::{JobExecutor, JobExecutionResult};
    ...
    pub async fn send_notification(
        ...
        if let Some(config) = &job.notification_config {
            ...
            for channel in &config.notification_channels {
                self.send_to_channel(channel, job, &event, message.as_deref()).await?;
            }
        }
    ...
    async fn send_to_channel(
        ...
        match channel {
            ...
            NotificationChannel::Log => {
                info!("日誌通知: 任務 {} - {:?} - {}", job.name, event, message.unwrap_or(""));
            }
        }
...
impl TaskMonitor {
    ...
    pub async fn record_execution(&self, result: JobExecutionResult) {
        ...
        // 更新指標
        {
            ...
            // 更新平均執行時間
            let total_time = metrics.average_execution_time_ms * (metrics.total_executions - 1) as f64 + duration_ms;
...
impl JobEngine {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        // 載入並排程所有活躍任務
        let jobs = self.job_repository.get_jobs_by_status(JobStatus::Active).await?;
        for job in jobs {
    ...
    pub async fn get_engine_state(&self) -> JobEngineState {
        ...
        if let Ok(all_jobs) = self.job_repository.get_all_jobs().await {
            ...
            state.active_jobs = all_jobs.iter().filter(|j| j.status == JobStatus::Active).count();
            state.running_jobs = all_jobs.iter().filter(|j| j.status == JobStatus::Running).count();
            state.paused_jobs = all_jobs.iter().filter(|j| j.status == JobStatus::Paused).count();
            state.failed_jobs = all_jobs.iter().filter(|j| j.status == JobStatus::Failed).count();
        }
