...
use crate::models::{
    ApiResponse, ClaudeRequest, ClaudeResponse, ExecutionStatus, Job, JobStatus, JobType,
...
async fn test_performance_benchmarks() {
    ...
    assert!(
        startup_duration.as_millis() < 500,
        "服務容器啟動應該在500毫秒內完成"
    );
    ...
    assert!(
        health_check_duration.as_millis() < 100,
        "健康檢查應該在100毫秒內完成"
    );
    ...
    assert!(
        shutdown_duration.as_millis() < 300,
        "服務容器停止應該在300毫秒內完成"
    );
