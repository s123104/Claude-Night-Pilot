...
impl HealthChecker {
            ...
            while *running_flag.read().await {
                ...
                
                {
                    ...
                    for (name, check) in checks_guard.iter_mut() {
                        // 檢查是否需要執行
                        let should_check = check.last_check
                ...
                
                {
                    ...
                    if !matches!(*status, HealthStatus::Healthy) || !matches!(new_status, HealthStatus::Healthy) {
                        info!(
    ...
    /// 註冊健康檢查
    pub async fn register_check(&self, name: String, check_fn: fn() -> HealthCheckResult, critical: bool) {
...
fn check_memory_usage() -> HealthCheckResult {
    ...
    details.insert("usage_percentage".to_string(), format!("{:.1}%", usage_percentage));
...
fn check_disk_space() -> HealthCheckResult {
    ...
    details.insert("total_space_gb".to_string(), format!("{:.1}", total_space_gb));
    details.insert("usage_percentage".to_string(), format!("{:.1}%", usage_percentage));
