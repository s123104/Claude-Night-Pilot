...
impl SimpleDatabase {
    ...
    pub fn create_prompt_with_tags(&self, title: &str, content: &str, tags: Option<&str>) -> Result<i64> {
    ...
    pub fn update_schedule(&self, id: i64, schedule_time: Option<&str>, status: Option<&str>, cron_expr: Option<&str>) -> Result<()> {
        ...
        let mut param_values: Vec<Box<dyn rusqlite::ToSql>> = vec![Box::new(Utc::now().to_rfc3339())];
...
mod tests {
    ...
    fn test_create_and_retrieve_schedule() {
        ...
        // 创建排程
        let schedule_id = db.create_schedule(prompt_id, "2025-01-01T09:00:00Z", Some("0 9 * * *")).unwrap();
    ...
    fn test_update_schedule() {
        ...
        let prompt_id = db.create_prompt("Test Prompt", "Test Content").unwrap();
        let schedule_id = db.create_schedule(prompt_id, "2025-01-01T09:00:00Z", None).unwrap();
        
        // 更新排程
        db.update_schedule(schedule_id, Some("2025-01-02T10:00:00Z"), Some("running"), Some("0 10 * * *")).unwrap();
    ...
    fn test_update_schedule_partial() {
        ...
        let prompt_id = db.create_prompt("Test Prompt", "Test Content").unwrap();
        let schedule_id = db.create_schedule(prompt_id, "2025-01-01T09:00:00Z", Some("original")).unwrap();
        
        // 只更新状态
        db.update_schedule(schedule_id, None, Some("completed"), None).unwrap();
    ...
    fn test_list_schedules() {
        ...
        let prompt_id = db.create_prompt("Test Prompt", "Test Content").unwrap();
        let id1 = db.create_schedule(prompt_id, "2025-01-01T09:00:00Z", None).unwrap();
        let id2 = db.create_schedule(prompt_id, "2025-01-02T10:00:00Z", None).unwrap();
    ...
    fn test_get_pending_schedules() {
        ...
        let prompt_id = db.create_prompt("Test Prompt", "Test Content").unwrap();
        let pending_id = db.create_schedule(prompt_id, "2025-01-01T09:00:00Z", None).unwrap();
        let completed_id = db.create_schedule(prompt_id, "2025-01-02T10:00:00Z", None).unwrap();
        
        // 更新一个为completed状态
        db.update_schedule_status(completed_id, "completed").unwrap();
    ...
    fn test_record_execution_result() {
        ...
        // 记录执行结果
        let result_id = db.record_execution_result(
            schedule_id,
    ...
    fn test_database_transaction_consistency() {
        ...
        // 记录执行结果应该自动更新排程统计
        db.record_execution_result(schedule_id, "Success", "success", Some(1000), Some(0.01), 1500).unwrap();
        ...
        // 再次执行
        db.record_execution_result(schedule_id, "Success again", "success", Some(1200), Some(0.012), 1800).unwrap();
    ...
    fn test_concurrent_access() {
        ...
        for i in 0..5 {
            ...
            let handle = thread::spawn(move || {
                ...
                let prompt_id = db.create_prompt(&format!("Title {}", i), &format!("Content {}", i)).unwrap();
                assert!(prompt_id > 0);
            });
            handles.push(handle);
        }
