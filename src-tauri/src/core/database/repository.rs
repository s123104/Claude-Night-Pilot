...
use crate::core::database::{
    ConnectionManager, DatabaseError, DatabaseResult, EntityId,
    Entity, Timestamped, Prompt, Job, ExecutionResult,
...
impl BaseRepository {
    ...
    /// 获取数据库连接
    pub fn get_connection(&self) -> DatabaseResult<std::sync::MutexGuard<rusqlite::Connection>> {
...
impl PromptRepository {
    ...
    pub async fn find_by_tags(&self, tags: &[&str]) -> DatabaseResult<Vec<Prompt>> {
        ...
        let conn = self.base.get_connection()?;
        let placeholders = tags.iter().map(|_| "?").collect::<Vec<_>>().join(" OR tags LIKE ");
        let sql = format!(
            "SELECT id, title, content, tags, created_at, updated_at 
             FROM prompts 
             WHERE tags LIKE {} 
        ...
        let params: Vec<&dyn rusqlite::ToSql> = tag_params.iter().map(|s| s as &dyn rusqlite::ToSql).collect();
...
impl Repository<Prompt> for PromptRepository {
    async fn create(&self, prompt: &mut Prompt) -> DatabaseResult<EntityId> {
        prompt.validate().map_err(|msg| DatabaseError::validation(msg))?;
    ...
    async fn update(&self, prompt: &mut Prompt) -> DatabaseResult<bool> {
        ...
        let id = prompt.id().ok_or_else(|| DatabaseError::validation("无法更新没有ID的实体"))?;
...
impl JobRepository {
    ...
    /// 更新任务状态
    pub async fn update_status(&self, id: EntityId, status: JobStatus, next_run_at: Option<DateTime<Utc>>) -> DatabaseResult<bool> {
        let conn = self.base.get_connection()?;
        ...
        let rows_affected = conn.execute(
            ...
            params![
                ...
                status.to_string(),
                if status == JobStatus::Running { Some(&now) } else { None },
                id
            ],
        ).map_err(DatabaseError::Connection)?;
    ...
    fn map_row_to_job(&self, row: &Row) -> rusqlite::Result<Job> {
        ...
        let updated_at = updated_at_str.as_ref().map(|s| parse_datetime(s)).transpose()?;
        let last_run_at = last_run_at_str.as_ref().map(|s| parse_datetime(s)).transpose()?;
        let next_run_at = next_run_at_str.as_ref().map(|s| parse_datetime(s)).transpose()?;
...
impl Repository<Job> for JobRepository {
    async fn create(&self, job: &mut Job) -> DatabaseResult<EntityId> {
        job.validate().map_err(|msg| DatabaseError::validation(msg))?;
    ...
    async fn update(&self, job: &mut Job) -> DatabaseResult<bool> {
        ...
        let id = job.id().ok_or_else(|| DatabaseError::validation("无法更新没有ID的实体"))?;
...
impl UsageRepository {
    ...
    /// 记录执行结果
    pub async fn record_execution(&self, _job_id: EntityId, result: &ExecutionResult) -> DatabaseResult<EntityId> {
        let mut exec_result = result.clone();
        self.create(&mut exec_result).await
    }
    
    /// 获取任务的执行结果
    pub async fn get_job_results(&self, job_id: EntityId, limit: Option<u32>) -> DatabaseResult<Vec<ExecutionResult>> {
...
impl Repository<ExecutionResult> for UsageRepository {
    async fn create(&self, result: &mut ExecutionResult) -> DatabaseResult<EntityId> {
        result.validate().map_err(|msg| DatabaseError::validation(msg))?;
    ...
    async fn list(&self, options: Option<QueryOptions>) -> DatabaseResult<PagedResult<ExecutionResult>> {
        let opts = options.unwrap_or_default();
        let limit = opts.limit.unwrap_or(50);
        let offset = opts.offset.unwrap_or(0);
