...
fn test_compile_time_query_verification() {
    ...
    // 創建測試數據
    let prompt = manager.with_connection(|conn| {
...
fn test_active_record_pattern() {
    ...
    let all_prompts = manager.with_connection(|conn| {
        RusqlitePrompt::find_all(conn, Some(10))
    }).unwrap();
    assert_eq!(all_prompts.len(), 3);
    
    // 測試find_by_id方法
    for id in &created_ids {
        let found = manager.with_connection(|conn| {
            RusqlitePrompt::find_by_id(conn, *id)
        }).unwrap();
        assert!(found.is_some());
    }
    
    // 測試delete方法
    let deleted = manager.with_connection(|conn| {
        RusqlitePrompt::delete(conn, created_ids[0])
    }).unwrap();
    assert!(deleted);
...
fn test_transaction_handling() {
    ...
    // 驗證事務提交成功
    let prompts = manager.with_connection(|conn| {
        RusqlitePrompt::find_all(conn, None)
    }).unwrap();
    let transaction_prompts: Vec<_> = prompts.iter()
...
fn test_advanced_search_functionality() {
    ...
    let test_prompts = vec![
        ("React Hook 使用", "React Hook最佳實踐", Some("react,hook,前端".to_string()), true),
        ("Rust 所有權", "Rust所有權系統詳解", Some("rust,所有權,後端".to_string()), false),
        ("SQL 優化", "數據庫查詢優化技巧", Some("sql,數據庫,優化".to_string()), true),
    ];
    ...
    // 測試標籤搜索
    let react_results = manager.with_connection(|conn| {
    ...
    // 測試收藏搜索
    let favorite_results = manager.with_connection(|conn| {
        RusqlitePrompt::search(conn, None, Some(true), None, None, None)
    }).unwrap();
    assert_eq!(favorite_results.len(), 2);
    
    // 測試內容搜索
    let optimization_results = manager.with_connection(|conn| {
        RusqlitePrompt::search(conn, None, None, Some("優化"), None, None)
    }).unwrap();
...
fn test_statistics_functionality() {
    ...
    let test_data = vec![
        ...
        ("中等內容", "這是中等長度的內容", Some("標籤2".to_string()), true),
        ("長內容", "這是很長很長的內容，用來測試平均長度計算功能", None, false),
