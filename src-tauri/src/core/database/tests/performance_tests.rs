...
fn test_insert_performance() {
    ...
    let small_result =
        PerformanceResult::new("單條插入", config.small_dataset_size, start.elapsed());

    // 驗證性能要求
    assert!(
        small_result.duration.as_millis() < 5000,
        "小數據集插入應在5秒內完成"
    );
    assert!(
        small_result.operations_per_second > 10.0,
        "插入速度應大於10 ops/sec"
    );

    println!(
        "✅ 小數據集插入: {} 條記錄，耗時 {:?}，速度 {:.1} ops/sec",
    ...
    let medium_result =
        PerformanceResult::new("單條插入", config.medium_dataset_size, start.elapsed());

    // 中等數據集性能要求稍微寬鬆
    assert!(
        medium_result.duration.as_millis() < 30000,
        "中等數據集插入應在30秒內完成"
    );
    assert!(
        medium_result.operations_per_second > 5.0,
        "插入速度應大於5 ops/sec"
    );

    println!(
        "✅ 中等數據集插入: {} 條記錄，耗時 {:?}，速度 {:.1} ops/sec",
...
fn test_concurrent_insert_performance() {
    ...
    assert!(
        concurrent_result.duration.as_millis() < 20000,
        "插入應在20秒內完成"
    );
    assert!(
        concurrent_result.operations_per_second > 5.0,
        "插入速度應大於5 ops/sec"
    );

    println!(
        "✅ 順序插入: {} 條記錄，耗時 {:?}，速度 {:.1} ops/sec",
        concurrent_result.dataset_size,
        concurrent_result.duration,
        concurrent_result.operations_per_second
    );
}
...
fn test_update_performance() {
    ...
    let update_result =
        PerformanceResult::new("單條更新", config.small_dataset_size, update_duration);

    assert!(
        update_result.duration.as_millis() < 5000,
        "更新操作應在5秒內完成"
    );
    assert!(
        update_result.operations_per_second > 5.0,
        "更新速度應大於5 ops/sec"
    );

    println!(
        "✅ 單條更新: {} 條記錄，耗時 {:?}，速度 {:.1} ops/sec",
    ...
    let updated_count = manager
        .with_connection(|conn| RusqlitePrompt::bulk_update_favorite(conn, &batch_ids, true))
        .unwrap();
...
fn test_delete_performance() {
    ...
    assert!(
        delete_result.duration.as_millis() < 3000,
        "刪除操作應在3秒內完成"
    );
    assert!(
        delete_result.operations_per_second > 10.0,
        "刪除速度應大於10 ops/sec"
    );

    println!(
        "✅ 單條刪除: {} 條記錄，耗時 {:?}，速度 {:.1} ops/sec",
...
fn test_statistics_performance() {
    ...
    println!(
        "✅ 統計查詢: 數據量 {} 條，耗時 {:?}",
        stats.total_count, stats_duration
    );
    println!(
    ...
    for benchmark in benchmarks {
        ...
        match benchmark.operation {
            "插入" => {
                ...
                assert!(
                    duration.as_millis() <= benchmark.max_duration_ms as u128,
                    "插入性能回歸：耗時 {}ms > 基準 {}ms",
                    duration.as_millis(),
                    benchmark.max_duration_ms
                );
                assert!(
                    ops_per_sec >= benchmark.min_ops_per_second,
                    "插入性能回歸：速度 {:.1} < 基準 {:.1} ops/sec",
                    ops_per_sec,
                    benchmark.min_ops_per_second
                );
