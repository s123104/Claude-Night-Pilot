...
use crate::core::database::best_practices_rusqlite::{
    ...
    PromptStatistics as RusqlitePromptStatistics, RusqliteBestPracticesConfig,
};
...
fn create_memory_config() -> RusqliteBestPracticesConfig {
    use crate::core::database::best_practices_rusqlite::{JournalMode, SynchronousMode};
...
async fn test_config_structure_validation() {
    ...
    assert_eq!(
        default_config.database_path.to_string_lossy(),
        "claude-pilot-best-practices.db"
    );
...
async fn test_backup_metrics_structure() {
    ...
    // 驗證結構完整性
    assert_eq!(
        backup_metrics.backup_path.to_string_lossy(),
        "/tmp/test_backup.db"
    );
...
async fn test_statistics_structure() {
    ...
    println!(
        "📊 統計示例: 收藏率={:.1}%, 標籤率={:.1}%",
        favorite_percentage, tagged_percentage
    );
}
...
async fn test_error_handling_logic() {
    ...
    fn complex_operation() -> Result<String> {
        simulate_database_error().map_err(|e| anyhow!("複雜操作失敗: {}", e))
    }
