// 数据库迁移管理器
use crate::core::database::{ConnectionManager, DatabaseError, DatabaseResult};
...
impl MigrationManager {
    ...
    pub fn migrate_to(&self, target_version: u32) -> DatabaseResult<Vec<u32>> {
        ...
        // 获取需要执行的迁移，按版本号排序
        let mut pending_migrations: Vec<&Migration> = self
            .migrations
            .values()
    ...
    pub fn rollback_to(&self, target_version: u32) -> DatabaseResult<Vec<u32>> {
        ...
        // 获取需要回滚的迁移，按版本号倒序
        let mut rollback_migrations: Vec<&Migration> = self
            .migrations
            .values()
    ...
    pub fn get_migration_history(&self) -> DatabaseResult<Vec<MigrationRecord>> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            // 解析迁移信息
            let migration_info: serde_json::Value =
                serde_json::from_str(&value_str).unwrap_or_else(|_| serde_json::json!({}));
    ...
    pub fn validate_database(&self) -> DatabaseResult<ValidationResult> {
        ...
        // 检查外键约束
        let foreign_key_enabled: bool = conn.query_row("PRAGMA foreign_keys", [], |row| {
