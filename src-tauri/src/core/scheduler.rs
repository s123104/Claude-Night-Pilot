    ...
    async fn schedule(&mut self, config: Self::Config) -> Result<Self::Handle> {
        ...
        let handle = tokio::spawn(async move {
            loop {
                ...
                // 計算下次檢查間隔
                let interval =
                    Self::calculate_adaptive_interval(&config.intervals, remaining_minutes);
...
impl Scheduler for SessionScheduler {
    ...
    async fn schedule(&mut self, config: Self::Config) -> Result<Self::Handle> {
        ...
        let tokio_duration = duration_until
            .to_std()
    ...
    async fn list_active(&self) -> Vec<Self::Handle> {
        self.sessions
            .iter()
            .filter(|(_, session)| {
                matches!(session.status, JobStatus::Scheduled | JobStatus::Running)
            })
            .map(|(id, _)| *id)
            .collect()
    }
...
mod tests {
    ...
    async fn test_cron_scheduler_reschedule() {
        ...
        let new_handle = scheduler
            .reschedule(original_handle, new_config)
            .await
            .unwrap();
    ...
    fn test_adaptive_scheduler_interval_calculation() {
        ...
        // 测试不同剩余时间的间隔计算
        assert_eq!(
            AdaptiveScheduler::calculate_adaptive_interval(&intervals, 70),
            Duration::from_secs(30)
        );
        assert_eq!(
            AdaptiveScheduler::calculate_adaptive_interval(&intervals, 45),
            Duration::from_secs(15)
        );
        assert_eq!(
            AdaptiveScheduler::calculate_adaptive_interval(&intervals, 15),
            Duration::from_secs(5)
        );
        assert_eq!(
            AdaptiveScheduler::calculate_adaptive_interval(&intervals, 5),
            Duration::from_secs(30)
        ); // 默认值
    }
