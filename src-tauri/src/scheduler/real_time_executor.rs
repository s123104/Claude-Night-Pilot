...
impl RealTimeExecutor {
        ...
        match scheduler.start().await {
            ...
            Err(e) => {
                ...
                Err(anyhow::anyhow!(
                    ...
                    4. Consult Context7 docs for latest best practices.", e
                ))
            }
        }
    }
    ...
    pub async fn add_job(&self, job: &Job) -> Result<String> {
        ...
        let cron_job = CronJob::new_async(cron_expression, move |_uuid, _l| {
            ...
            Box::pin(async move {
                ...
                info!("🎯 Executing scheduled job: {} (Prompt ID: {})", job_name, prompt_id);
                ...
                // 更新統計數據
                {
                    ...
                    if let Some(job_stats) = stats.get_mut(&job_id) {
                        ...
                        job_stats.average_duration_ms = 
                            (job_stats.average_duration_ms * (job_stats.total_runs - 1) as f64 + duration_ms) 
                            / job_stats.total_runs as f64;
                        
                        match execution_result {
                            Ok(ref exec_result) => {
                                job_stats.successful_runs += 1;
                                info!("✅ Job {} completed successfully in {}ms (Execution: {})", 
                                     job_id, duration.as_millis(), exec_result.execution_id);
                            }
                            Err(ref e) => {
                                job_stats.failed_runs += 1;
                                error!("❌ Job {} failed after {}ms: {}", job_id, duration.as_millis(), e);
                            }
                        }
                    }
                }
        ...
        // 添加到排程器
        let scheduler = self.scheduler.lock().await;
        let cron_uuid = scheduler.add(cron_job).await
        ...
        info!("📅 Added scheduled job: {} with cron: {}", job.name, job.cron_expression);
        Ok(job.id.clone())
    }
    ...
    fn validate_cron_expression(&self, cron_expr: &str) -> Result<()> {
        ...
        if parts.len() != 5 {
            return Err(anyhow::anyhow!(
                "Invalid cron expression format. Expected 5 parts (min hour day month weekday), got {}",
                parts.len()
            ));
        }
    ...
    async fn execute_job_logic(job_id: &str, prompt_id: &str) -> Result<JobExecutionResult> {
        ...
        info!("🔄 Starting job execution: {} (Prompt: {}, Execution: {})", job_id, prompt_id, execution_id);
        
        // Phase 1: 檢索 prompt 內容
        let prompt_content = match Self::retrieve_prompt_content(prompt_id).await {
            Ok(content) => content,
            Err(e) => {
                warn!("⚠️ Failed to retrieve prompt content for prompt_id {}: {}", prompt_id, e);
        ...
        info!("📝 Executing prompt content (length: {} chars)", prompt_content.len());
        info!("📋 Prompt preview: {}", 
        ...
        let result = JobExecutionResult {
            ...
            output: Some(format!("Prompt executed successfully. Content length: {} chars", prompt_content.len())),
            error_message: None,
            duration_ms,
            start_time,
            end_time,
        ...
        info!("✅ Job {} execution completed successfully in {}ms (Execution: {})", 
    ...
    async fn retrieve_prompt_content(prompt_id: &str) -> Result<String> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = conn.prepare("SELECT content FROM prompts WHERE id = ?")
                .context("Failed to prepare statement")?;
            
            let content: String = stmt.query_row([&prompt_id], |row| {
    ...
    /// 儲存執行結果
    async fn save_execution_result(job_id: &str, prompt_id: &str, result: &JobExecutionResult) -> Result<()> {
...
mod tests {
    ...
    async fn test_cron_validation() {
        ...
        // 有效表達式
        assert!(executor.validate_cron_expression("0 9 * * 1").is_ok());
    ...
    async fn test_job_lifecycle() {
        ...
        let job = Job {
            ...
            cron_expression: "*/5 * * * *".to_string(), // 每5分鐘
