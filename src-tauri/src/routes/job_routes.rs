...
pub async fn list_jobs(
    ...
    // 模擬任務數據
    let mock_jobs = vec![
        Job::new("Daily Report Job", "prompt_1", "0 9 * * *")
            .with_priority(5),
        Job::new("Code Review Job", "prompt_2", "0 */2 * * *")
            .with_priority(7),
        Job::one_time("Immediate Analysis", "prompt_3", chrono::Utc::now() + chrono::Duration::minutes(30))
            .with_priority(9),
    ];
    
    // 根據狀態過濾
    let filtered_jobs: Vec<Job> = if let Some(filter_status) = status {
        mock_jobs.into_iter().filter(|job| job.status == filter_status).collect()
    } else {
        mock_jobs
    };
...
pub async fn create_job(
    ...
    let mut job = match request.job_type {
        JobType::OneTime => Job::one_time(&request.name, &request.prompt_id, chrono::Utc::now() + chrono::Duration::hours(1)),
...
pub async fn update_job(
    ...
    // 模擬更新
    let mut job = Job::new(
        &request.name.unwrap_or_else(|| format!("Updated Job {}", id)),
        &format!("prompt_{}", id.chars().take(3).collect::<String>()),
        &request.cron_expression.unwrap_or_else(|| "0 9 * * *".to_string())
    );
    job.id = id.clone();
...
pub async fn cancel_job(
    ...
    match claude_service.cancel_execution(&id).await {
        ...
        Err(e) => {
            let error_response: ApiResponse<()> = ApiResponse::error(format!("取消任務失敗: {}", e));
