...
impl TauriAdapter {
    ...
    ) -> Result<ApiResponse<PaginatedResponse<crate::services::prompt_service::PromptServiceResponse>>> {
    ...
    pub async fn gui_create_prompt(
        ...
        match self.prompt_service.create_prompt(request.clone()).await {
            Ok(id) => {
                // 觸發UI狀態更新
                self.notify_ui_state_change("prompt_created", serde_json::json!({
                    "id": id,
                    "request": request
                })).await?;
    ...
    /// GUI專用的同步操作
    pub async fn gui_get_sync_status(&self) -> Result<ApiResponse<crate::services::sync_service::SyncStatus>> {
    ...
    /// GUI專用的健康檢查
    pub async fn gui_health_check(&self) -> Result<ApiResponse<crate::services::health_service::HealthStatus>> {
        let context = self.create_gui_context(OperationType::Read);
    ...
    /// 應用分頁邏輯
    fn apply_pagination<T: Clone>(&self, items: Vec<T>, pagination: Option<PaginationRequest>) -> (Vec<T>, usize) {
        let total = items.len();
    ...
    /// 通知UI狀態變更
    async fn notify_ui_state_change(&self, change_type: &str, data: serde_json::Value) -> Result<()> {
        // 發送狀態變更事件
    ...
    async fn notify_ui_refresh_all(&self) -> Result<()> {
        self.notify_ui_state_change("refresh_all", serde_json::json!({
            "timestamp": chrono::Utc::now().to_rfc3339()
        })).await
    }
