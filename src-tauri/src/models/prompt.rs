...
impl Prompt {
    ...
    pub fn add_file_reference(&mut self, file_ref: FileReference) {
        // 檢查是否已存在同名引用
        if let Some(existing) = self.file_references.iter_mut().find(|f| f.name == file_ref.name) {
            *existing = file_ref;
        } else {
    ...
    pub fn record_usage(&mut self, execution_time_ms: Option<u64>, success: bool) {
        ...
        if let Some(exec_time) = execution_time_ms {
            self.avg_execution_time_ms = Some(
                if let Some(avg) = self.avg_execution_time_ms {
        ...
        self.success_rate = Some(
            (current_success_rate * (self.usage_count - 1) as f64 + success_value) / self.usage_count as f64
        );
...
mod tests {
    ...
    fn test_prompt_variable_rendering() {
        ...
        prompt.add_variable(
            PromptVariable::new("name", VariableType::Text).required()
        );
        prompt.add_variable(
            PromptVariable::new("age", VariableType::Number).with_default("25")
        );
    ...
    fn test_prompt_validation() {
        ...
        prompt.add_variable(
            PromptVariable::new("email", VariableType::Email)
